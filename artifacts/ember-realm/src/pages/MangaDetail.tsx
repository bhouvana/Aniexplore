...
import { Star, BookOpen, Clock, User, Pen } from "lucide-react";
...
export default function MangaDetail() {
  ...
  const chapters = useGetMangaChapters(id, {
  ...
  const firstChapter = chapters.data?.chapters[chapters.data.chapters.length - 1];
  const latestChapter = chapters.data?.chapters[0];
            ...
            <div className="flex gap-3 mt-4">
              {firstChapter && (
                <Link href={`/manga/read/${id}/${firstChapter.id}`}>
                  <Button
                    ...
                    className="bg-secondary hover:bg-secondary/90 text-white font-bold shadow-[0_0_15px_rgba(34,211,238,0.4)]"
                  >
              ...
              {latestChapter && latestChapter.id !== firstChapter?.id && (
                <Link href={`/manga/read/${id}/${latestChapter.id}`}>
                  <Button data-testid="read-latest-chapter" variant="outline" className="border-border">
              ...
              : chapters.data?.chapters.map((ch) => (
                  <Link key={ch.id} href={`/manga/read/${id}/${ch.id}`}>
                    <div
                      ...
                      className="flex items-center justify-between px-4 py-3 rounded-xl bg-card border border-border hover:border-secondary/30 hover:bg-secondary/5 transition-all cursor-pointer group"
                    >
                      <div>
